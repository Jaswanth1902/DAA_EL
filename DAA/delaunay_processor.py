...
class DelaunayProcessor:
    ...
    def _get_seeds(self, k=15, b_dist=5, interior_density=0.001):
        ...
        # 4. Interior random point insertion (Adaptive via Saliency Rejection Sampling)
        # Generate a saliency map to place more points in detailed areas and fewer in flat areas
        gray = cv2.cvtColor(small_img if max(self.h, self.w) > max_dim else self.image, cv2.COLOR_BGR2GRAY)
        laplacian = cv2.convertScaleAbs(cv2.Laplacian(gray, cv2.CV_64F))
        saliency = cv2.GaussianBlur(laplacian, (15, 15), 0).astype(np.float32)
        if max(self.h, self.w) > max_dim:
            saliency = cv2.resize(saliency, (self.w, self.h))
            
        saliency_norm = saliency / (np.max(saliency) + 1e-5)
        
        # Over-generate candidates and filter based on local saliency
        num_candidates = int(self.w * self.h * interior_density * 4) 
        cand_x = np.random.randint(0, self.w, num_candidates)
        cand_y = np.random.randint(0, self.h, num_candidates)
        
        # Accept points probabilistically: higher chance in high saliency areas
        accept_probs = saliency_norm[cand_y, cand_x] * 0.85 + 0.15 # Minimum 15% chance anywhere
        accept_mask = np.random.rand(num_candidates) < accept_probs
        
        interior_x = cand_x[accept_mask]
        interior_y = cand_y[accept_mask]
